...
1. **DERIVE `{req_name}`**: If the input references an ADO work item (Bug/Task/US/Feature + ID), load **`skill-aproda-ado`** and apply its Pattern 1 (e.g. Bug 36370 → `bug-36370`). Otherwise: lowercase, replace spaces and special characters with hyphens, collapse repeated hyphens, trim to ≤40 chars (e.g. "Customer VIP Program" → `customer-vip-program`). If only an ADO ID/URL is given without a title, call `Get-AdoWorkItem.ps1` (skill-aproda-ado's SRP-safe execution) instead of asking the user to paste it. State the derived name and let the user correct it before file creation. Then apply **Pattern 3**: if `.github/plans/{req_name}/` already exists, stop and ask whether to extend that plan or choose a different `{req_name}` — do not create files until the user decides.
