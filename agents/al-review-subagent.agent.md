...
tools: [read/problems, read/readFile, search, 'al-symbols-mcp/*', ms-dynamics-smb.al/al_debug, ms-dynamics-smb.al/al_setbreakpoint, ms-dynamics-smb.al/al_snapshotdebugging, ms-dynamics-smb.al/al_symbolsearch, ms-dynamics-smb.al/al_getdiagnostics, ms-dynamics-smb.al/al_symbolrelations, SShadowSdk.al-lsp-for-agents/bclsp_symbolRelations, SShadowSdk.al-lsp-for-agents/bclsp_goToDefinition, SShadowSdk.al-lsp-for-agents/bclsp_findReferences, SShadowSdk.al-lsp-for-agents/bclsp_hover, SShadowSdk.al-lsp-for-agents/bclsp_documentSymbols, SShadowSdk.al-lsp-for-agents/bclsp_codeQualityDiagnostics, aprodaag.aproda-aldc/aldcConfiguration, aprodaag.aproda-aldc/bcquality]
...
BCQuality is a curated, citable BC knowledge base reached through `#bcquality` or, as a fallback, by path (dual path, per `aldc.yaml`). It is a citation/audit layer — it does not replace the checklist or the auto-applied instructions; it adds findings backed by a knowledge file.
...
>
> **Standalone fallback only** (no decision passed — you were invoked outside the Conductor): resolve the switch and the clone in this order. (1) `#bcquality` (`read`, `skills/entry.md`) — `disabled`/`unresolved` → skip as above; `ok` → the content **is** the mounted signal, already in hand, proceed. (2) Tool unavailable → `#aldcConfiguration` for `external.bcquality.enabled` (absent ⇒ `auto`) + `resolvedHome`; `false` → skip; `auto`/`true` → **probe once**, `read_file <resolvedHome>/<entryPoint>`. (3) Both unavailable → `read_file <toolkitRoot>/aldc.yaml` for the declared `home` (a static default, may be wrong) and probe `<home>/<entryPoint>` before trusting it — never assume. Any probe that **errors or returns empty = absent** → skip as above — **never retry the read or proceed to Route/Execute**. A missing knowledge layer **never** fails or blocks the review.
