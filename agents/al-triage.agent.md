...
3. **Root-cause.** Trace backwards from the symptom (skill-debug): `al_debug` / `al_setbreakpoint` / `al_snapshotdebugging` for runtime, `al_getdiagnostics` + `bclsp_codeQualityDiagnostics` for compile/quality. Evidence, not guesses.
...
5. **Knowledge (optional, cited).** **Resolve the switch and the clone, then probe — never assume.** Three rungs, in order: (1) `#bcquality` (`read`, `skills/entry.md`) — `disabled`/`unresolved` → skip this step (rely on skill-debug + auto-applied instructions), do **not** probe or read the clone further; `ok` → content already in hand, consult it scoped to the suspect area (execute its `dispatch[]` → cited findings). (2) Tool unavailable → `#aldcConfiguration` for `external.bcquality.enabled` (absent ⇒ `auto`) + `resolvedHome`; `false` → skip; `auto`/`true` → **attempt** `read_file <resolvedHome>/<entryPoint>`. (3) Both unavailable → `read_file <toolkitRoot>/aldc.yaml` for the declared `home` (a static default, may be wrong) and probe `<home>/<entryPoint>` before trusting it. Fold any citations into Root Cause / Recommended Fix. A probe that **errors or returns empty counts as absent** — skip silently (don't retry) — skill-debug + the auto-applied instructions carry the knowledge (same graceful degradation the review subagent uses). For a broad "is this whole module unhealthy?" question, recommend a standalone **`@dredd`** audit instead. **Status — one line** (product signal): entry reads → `🟢 BCQuality · active — {ref, or 📌 <sha> if pinned}`; probe fails → `⚪ BCQuality · not mounted — native (skill-debug + instructions)`. Add `📎 BCQuality · {n} cited` to the diagnosis when citations exist.
6. **Diagnose.** If the ticket originates from an ADO work item, load **`skill-aproda-ado`** to derive the folder name and embed the ADO header — if only an ADO ID/URL is given without a title, perform a Tier-4 read (MCP or `Invoke-AdoAzCli.ps1`) instead of asking the user to paste it, and apply Pattern 3 (existing-plan hard stop) before creating any files. Write `.github/plans/<issue-kebab-case>/<issue-kebab-case>-diagnosis.md` using **skill-debug's Step 4 template**, plus two fields it under-specifies: **Blast radius** (from step 4) and **Citations** (BCQuality `file:line` from step 5, when present). Recommend a **minimal permanent fix** at the root cause; add a **short-term mitigation/hotfix** only when the permanent fix is risky or slow to ship.
