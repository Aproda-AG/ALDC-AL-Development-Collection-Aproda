...
- **Diagnostics**: **`al_getdiagnostics`** (filtered Problems) + **`bclsp_codeQualityDiagnostics`**.
...
4. **Build & validate** — build in the terminal (`execute`), read diagnostics via `al_getdiagnostics` + `bclsp_codeQualityDiagnostics`, fix and rebuild until clean. Run tests when they exist; on failure, fix and retest. Stuck after 3 build attempts → pause. For runtime/intermittent bugs use `al_debug` / `al_setbreakpoint` / `al_snapshotdebugging` and load `skill-debug`; for slow code apply `al-performance.instructions.md` then load `skill-performance`. **Translation gate:** when an app with `TranslationFile` enabled changes labels, captions, tooltips, or XLIFF files, load `skill-translate`; content-load the Aproda XLIFF tool and run `Sync -SkipBuild`, then `Resolve -ReportPath <shared-run-report>` (deterministic tier 1/tier 2 resolution — must run before `ExportOpen`, never after). Then repeat `ExportOpen -MaxItems <n> -Offset <n>`, advancing `Offset` by each emitted count, until reported `Remaining` is zero. For every batch, delegate the absolute batch and response paths to `AL Translation Subagent`, then run `Apply` yourself with a shared `-ReportPath`, because the subagent never writes XLIFF files. On an `Apply` rejection, retry the same batch with the rejection details at most twice; then stop and report the failure. `Apply` writes AI output as `needs-review-translation`; review all batches in PoEdit, then require `Validate -FailOnIssues -FailOnUnapproved`. Finally run `Report -ReportPath <shared-run-report>` to produce the correction rate, and record it with the approval-gate result, open review count, and `Resolve`'s tier 1/tier 2 counts. **Before handing off for PR**, when a BC service is reachable, load `skill-aproda-deploy-run-verify` and run the Deploy-Run-Verify Cycle until green or a real blocker (LOW-complexity trigger). **At the delivery boundary** (work accepted / ready for PR): (a) update `memory.md` → Active Requirements Status from `in progress` → `review` (begins the HITL Validation phase — see `hitl-validation.aproda.instructions.md`) + append Inter-Session Context entry; (b) if the change affects a documented module under `.github/documentation/<Module>/`, run `@workspace use al-doc-update` to refresh the technical reference + de-CH handbook (Aproda D-14).
