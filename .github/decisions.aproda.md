...
Fkh deployment is a reusable infrastructure capability, not a second lifecycle. `skill-aproda-deploy-run-verify` therefore remains responsible for environment acknowledgement, Build → Deploy → Run → Review sequencing, and the quality conclusion. Net-new `skill-aproda-fkh` owns only Fkh target verification, dependency sorting, publish/sync/install, and state inspection (D-4/D-16); it is loaded by the lifecycle skill when the selected target is Fkh.

The initial classifier is deliberately narrow: an acknowledged `launch.json` configuration is an Fkh candidate only if it is `OnPrem`, uses HTTPS, and its server hostname ends in `.cloudapp.azure.com`. It must then be verified against `fkh listcontainers`; an unmatched or ambiguous candidate stops for HITL rather than falling through to ASINST. This is a pragmatic first discriminator, not a permanent hostname taxonomy.

The existing ASINST engine remains first-class and unchanged. It is the only adapter with a proven complete test-runner and result-parsing flow (D-8/D-15). Fkh container discovery, sorting, publish, sync/install, and state inspection are validated, but Fkh test execution/result retrieval is explicitly marked **not validated**; a successful Fkh deployment is never described as a green full lifecycle.

Rejected: embedding Fkh command mechanics into the lifecycle skill (duplicates reusable transport knowledge), and replacing the ASINST engine with Fkh (would discard a validated adapter for an incomplete one). This is an additive skill plus a controlled lifecycle routing extension; no new Upstream touch-point is required.

**Live validation addendum (2026-09-11).** A complete companion-app cycle validated FKH runtime execution and established three operational facts. `listcontainers` matches the launch host through `webClient`, but later operations (`publishapp`, `getappinfo`, and `invokescript`) require the matched `appLabel`, not the Kubernetes deployment `name`. On a developer workstation, `--useOIDC` is invalid because it suppresses local authentication and requires CI OIDC variables; FKH must use the authenticated `gh` CLI session instead. FKH rejects re-publishing an existing app ID/version before it can synchronize, so an acknowledged development replacement removes dependent apps first, then uses `Uninstall-NAVApp` and `Unpublish-NAVApp` before sequential `ForceSync` publication. A SRP-safe shared web-client runner completed 18/18 focused tests and parsed its XUnit XML, making the FKH Run and Verify stages validated.
...
The engine (`AprodaDeployRunVerify.psm1`) was ASINST-only: `serviceUrl` was hardcoded to `http://`, the test run always used `-AutorizationType 'Windows'`, deploy always opened `New-PSSession` + the NAV Management DLL, and preflight used ICMP (`Test-Connection`), which an Azure load balancer commonly drops even when the web client answers — silently downgrading a healthy Fkh target to build-only. This meant the DRV skill's documented Fkh routing row was not actually executable through the entry point; every FKH-validated run had come from ad-hoc scripts under `Test/PowerShell/_temp/`, not the engine.

Fixed, following the D-26 adapter boundary (Fkh owns transport mechanics, Deploy-Run-Verify owns the loop):
- `scheme` and `authentication` are derived from the selected `launch.json` configuration (defaulting to `http`/`Windows` when absent), and `serviceUrl` is built from `scheme` instead of a hardcoded protocol.
- `Get-DeployRunVerifyAdapter` classifies `Asinst` vs `Fkh` using the same hostname rule as the skill (`https` + `*.cloudapp.azure.com`); `Invoke-DeployRunVerifyDeploy` is now a thin dispatcher over `Invoke-DeployRunVerifyDeployAsinst` (renamed, behavior unchanged) and the new `Invoke-DeployRunVerifyDeployFkh` (resolves the Fkh backend URL, matches the container via `fkh listcontainers` by `webClient` host, publishes each app in order via `fkh publishapp --syncMode ForceSync --sync --install`, and checks `fkh getappinfo`'s `apps[].IsInstalled`/`SyncState` for the matched version to confirm install/sync state (characterized 2026-09-13 from a live capture; an unmatched app entry warns rather than throwing a false `DEPLOY INCOMPLETE`).
- Preflight now sends an HTTP(S) HEAD request to `serviceUrl` instead of ICMP; any HTTP response (even 4xx/5xx) counts as reachable, only a transport-level failure does not.
- `Invoke-DeployRunVerifyRun` uses `Cfg.authentication`; for `UserPassword` it prompts once via `Get-Credential` (never from config, chat, or environment) and passes the secret to the child test-runner process as a DPAPI-protected string (`ConvertFrom-SecureString`, decryptable only by the same user/machine), cleared from the environment immediately after the run.

**Live-validated (2026-09-12).** A real run through this entry point (`skipbuild` mode) against `flobi-wan-kenobi-straub-dev-deployment` reached **26/26 passed, 0 failed** on the DC Blob Proxy Tests suite (codeunit 59007) — matching the previously recorded ad-hoc evidence exactly, now proven through the engine itself. Two real defects surfaced and were fixed during this run: (1) `Set-StrictMode -Version Latest` throws when `Get-DeployRunVerifyCredential` reads a never-initialized `$Script:` variable on its first call — fixed by initializing it at module load; (2) the same-version-conflict removal fallback needed its first live exercise to confirm the `fkh invokescript` Uninstall/Unpublish sequence actually clears the conflict for apps within `Cfg.apps` (confirmed). `Get-Credential` (console-based prompting, `ConsolePrompting` registry preference) correctly prompted once; the user typed credentials directly into the terminal, never exposed to the agent or chat, and the DPAPI pass-through to the child test-runner process worked as designed.

**New operational discovery requiring an explicit HITL policy.** The same-version removal fallback can itself fail when a **third-party app outside the current project's `Cfg.apps`** (e.g. a companion app from a parallel workstream sharing the same disposable dev container) depends on the app being removed — encountered live when `Straub Medical AG Document Capture Azure File Proxy` (an unrelated workstream's app) blocked Base's unpublish. The engine correctly failed loud with the underlying Fkh error rather than guessing, and resolving it required a one-off, manually run, human-approved removal script outside the engine — not an automatic action. This is deliberate, not a gap to silently close: auto-removing apps outside the project's own scope on a shared container is exactly the destructive, hard-to-reverse action that requires confirmation. **Going forward, whenever the Fkh same-version conflict resolution hits a dependent app outside `Cfg.apps`, the agent must stop and ask the user among exactly three options**: (a) remove only the blocking dependents and publish just the current app; (b) remove the app and its dependents and reinstall everything afterward; (c) abort for manual review. The engine's automatic fallback intentionally stays scoped to `Cfg.apps` only — it never expands its removal set on its own.

**Also fixed as part of this pass**: the duplicate `D-26` heading (two unrelated decisions shared the number) was renumbered — the decision "HITL gates must be self-verified, not just narrated" is now **D-35**; its two existing cross-references were updated. A plaintext dev-container credential committed to a tracked plan file (`.github/plans/azure-function-proxy-file/orchestrator auftrag.md`) was removed from the file (already in git history; the disposable container should be deleted or its password rotated, per the plan's own stated intent).

Rejected: leaving preflight on ICMP with a documented caveat — rejected because a silent build-only downgrade produces a false-green report with no visible signal, which the engine's own deploy stage already treats as unacceptable (`DEPLOY INCOMPLETE` fail-loud). Embedding the Fkh CLI calls inside `skill-aproda-fkh` instead of the engine was also considered, but the skill ships no reusable PowerShell module today (only documentation + one helper script); duplicating its documented command shapes into the engine is the pragmatic interim, not a second source of truth for the *rules* (hostname classification, `appLabel` usage, no `--useOIDC` locally), which remain owned by the skill and are only executed here.

### D-37 — Fkh credential caching lives in skill-aproda-fkh; the DRV engine consumes it, with a live-confirmed retry-on-ambiguous-failure policy

Every `UserPassword` run against an Fkh target re-prompted via `Get-Credential`, with no way to cache a credential across sessions, and no recovery path when a connection attempt failed for any reason. Two things needed solving: where a reusable credential cache belongs, and how to react when a connection never comes up.

**Cache ownership and shape.** Per D-26 (Fkh owns transport/auth mechanics; Deploy-Run-Verify only consumes them), the cache is a new `skill-aproda-fkh/scripts/FkhCredentialStore.ps1` — not engine-owned — with three functions (`Get-FkhStoredCredential`, `Save-FkhCredential`, `Remove-FkhStoredCredential`). It stores one DPAPI-protected `Export-Clixml` file per Fkh container **hostname** under `%LOCALAPPDATA%\AprodaFkh\credentials\` — workstation-scoped like `fkh.backendUrl` (D-33), not project-local, and keyed per-container so multiple Fkh targets with the same or different passwords never collide. The DRV engine loads it content-safe (SRP-exempt, same pattern as the runner glue) and only as a thin consumer: check the store first, then prompt via `Get-Credential` on a miss. After a run reaches `stage='ran'`, a freshly entered credential is saved or updated automatically; this policy is recorded in the 2026-09-13 revision below.

**This is the engine's first runtime dependency reaching into a sibling skill's `scripts/` folder.** Deliberate and consistent with D-26's ownership split, not a new pattern to be applied loosely elsewhere.

**Retry policy — live-confirmed, not assumed (2026-09-13).** A connection attempt that never reaches `stage='ran'` surfaces only as `System.Management.Automation.RuntimeException: ClientSession State is Uninitialized (Wait time 10 seconds)` — reproduced 2-for-2 with a deliberately wrong password against `flobi-wan-kenobi-straub-dev-deployment`, and absent on a correct-password run in between (`stage='ran'`, 26/26). This message is **not** a distinguishable auth-failure signal (no `401`/`Unauthorized`/localized login-failed text anywhere in `$res.console` or `$res.error`) — it is a generic "the session never became ready" timeout that can just as easily mean an unreachable service, a misnamed company, or a network hiccup. Building a text-pattern classifier for "wrong password" specifically was rejected as unreliable; the only trustworthy signal is structural: did `stage` ever leave `'start'`.

Chosen policy: on `stage != 'ran'` for a `UserPassword` target, clear the cached credential (`Reset-DeployRunVerifyCredential`) and retry **exactly once** with a freshly re-prompted credential; a second consecutive failure is a real blocker (stop, surface the original error, no further retry — consistent with the loop's "loop until green or a real blocker" rule, never brute-force). Once a run reaches `stage='ran'` (regardless of AL test pass/fail counts), a freshly entered credential is saved or updated automatically because the connection itself is proven to work.

Rejected: a regex-based auth-failure detector (no reliable string exists, live-confirmed); storing credentials keyed by Fkh `appLabel` instead of hostname (hostname is already known before `appLabel` is resolved, avoiding an ordering dependency, and is equally 1:1 with a given container in practice). The initially rejected automatic persistence policy was revisited and reversed in the same-day revision below after a freshly entered credential had been proven against the service.

**Live-validated (2026-09-13).** All three scenarios were exercised for real against `flobi-wan-kenobi-straub-dev-deployment`, in this order: (1) empty store → fresh `Get-Credential` prompt → 26/26 → saved under the then-current opt-in behavior; (2) populated store → silent reuse ("Using stored Fkh credential...", no prompt) → 26/26; (3) store deliberately overwritten with a wrong password → attempt 1 loads it silently and fails (`stage='start'`) → retry warning → attempt 2 forces a fresh prompt (store correctly bypassed) → correct password → 26/26 → store updated. One real bug was found and fixed before these passed: dot-sourcing `FkhCredentialStore.ps1` from *inside* a helper function (`Import-FkhCredentialStoreFunctions`) only bound its functions into that function's own scope, which vanished the instant it returned — `Get-FkhStoredCredential` was then "not recognized" from any caller. Fixed by loading the store content eagerly at the engine's module top level instead (same scope as every other top-level function definition), guarded by a try/catch so a pure-ASINST setup without `skill-aproda-fkh` just warns instead of failing to load.
