...
**Scope correction (E-006 T-33 / D-49).** The quoted premise about `.github` breaking sync, bootstrap, and the fleet contract is **superseded**: D-49 moved `aldc.yaml` to `<toolkitRoot>/aldc.yaml` (`.github/` in a consuming project) and updated sync, bootstrap, and the fleet scripts to a two-rung probe that supports exactly that location. The rest of D-24's rationale — the tool exists because the repository root is often not a workspace folder — still holds.
...
| `tools/aldc-validate/index.js` | **E-006 T-33**: without `--config` the validator now resolves `aldc.yaml` two-rung (`.github/aldc.yaml` → repo root) instead of assuming the repo root. `--config` is unchanged and still wins. Needed because `aldc.yaml` moves to `toolkitRoot`, which is `.github` in a consuming project and `.` in the fork — the fork/upstream layout therefore keeps resolving exactly as before. | D-2 / D-49 | 2026-09-25 |
| `tools/aproda-sync/**` (Aproda net-new: `aproda-sync.json`, `Sync-AprodaLayer.ps1`, `Bootstrap-AprodaProject.ps1`, `Test-InPlaceEditsRegister.ps1`, `fleet/Get-AprodaFleetStatus.ps1`, `fleet/Start-FleetGather.ps1`, `fleet/Start-FleetUpdate.ps1`, `templates/gitignore-block.txt`) | **E-006 T-33**: `aldc.yaml` moves to `<toolkitRoot>/aldc.yaml`. The `dualVariant` entry gained `sidePaths` (`fork: aldc.yaml`, `project: .github/aldc.yaml`) so the file's *physical* path may diverge per side, not only its `toolkitRoot` line; every reader resolves two-rung (`.github/` first, repo root second), so the pre-T-33 layout keeps working unmigrated. The gitignore block now carries **both** `/.github/aldc.yaml` and `/aldc.yaml`, so a not-yet-migrated project's stale root copy stays ignored. **Scope correction:** T-33 listed 3 affected sites; the implementation found **8** — the three fleet scripts each carried the now-false comment *"aldc.yaml always sits at the REPO ROOT on both fork and project side"*. Verified by `-WhatIf` pull into `straub-medical-ag-base`: `Dual-variant: aldc.yaml -> .github/aldc.yaml`, 130 layer files (unchanged vs. the T-31 baseline), no warnings. Net-new Aproda files — no `inPlaceEdits` entry needed. **Removal of the stale root copy is T-35, not this change.** | D-4 / D-18 / D-49 | 2026-09-25 |
| `tools/aproda-sync/templates/workspace.seed.jsonc`, `Initialize-AprodaProject.ps1`, `templates/gitignore-block.txt`, `aproda-sync.json` (+ new net-new `templates/external-readme.seed.md`) | **E-006 T-29**: swaps the BCQuality workspace root from the sibling clone (`../BCQuality-Aproda`) to the tracked `.external/` wrapper, with `search.exclude` + `files.watcherExclude` on `bcquality/**` (never `files.exclude`) — the mount can now be excluded from search because the junction sits *inside* the mounted root instead of *being* it (B-9). Both of `Initialize-AprodaProject.ps1`'s workspace writers (the missing-root patcher and the from-scratch fallback creator) were updated so no code path is left on the old layout, and it now also seeds `.external/README.md` from the new template on first run (idempotent, never overwrites). `gitignore-block.txt` gained `/.external/bcquality/` inside the marker block; the wrapper folder and its `README.md` stay tracked. **The junction itself is not created here** — that is the Aproda VS Code extension's job (a later step). **Existing projects keep their `../BCQuality-Aproda` root untouched** — migrating them onto `.external/` is T-35, deliberately not part of this change. Net-new Aproda files/templates — no `inPlaceEdits` entry needed for the new template; `Initialize-AprodaProject.ps1` is already registered in `includeFiles`. **`workspace.seed.jsonc` is deliberately NOT registered and does not ship** — it is fork-side reference documentation; the shipping code path is `Initialize-AprodaProject.ps1`'s fallback writer, which builds the same JSON inline. Review follow-up: `docs/bcquality.md`, which the new `.external/README.md` links to, was **not** in `aldc.yaml → required.catalog` and therefore did not ship either — registered in the same pass, same mechanism as `docs/copilot-reference.md` (T-14/F-11). | D-4 / D-49 | 2026-09-25 |
| `tools/aproda-sync/Sync-AprodaLayer.ps1` | **E-006 T-29 review finding — a shipped defect, found by adjudicating why `docs/bcquality.md` did not appear in a dry-run.** The framework-file scrape anchored its regex on the closing quote (`"\s*$`), so **any `required`/`optional`/`catalog` entry carrying a trailing `# comment` was silently dropped** and never shipped. That is exactly the three most recently registered catalog files — `agents/index.md` (T-11), `docs/copilot-reference.md` (T-14/F-11) and `docs/bcquality.md` (T-29) — so **T-11's and T-14's "now it ships" claims were not in effect**, the same failure class those items were opened to fix. Regex now tolerates a trailing comment; measured against this repo's `aldc.yaml`: **73 → 76** matched entries, the delta being exactly those three lines. Pre-existing defect, not introduced by T-29. **Note:** a catalog change still needs a *second* pull to take effect, because the scrape reads the **destination's** `aldc.yaml`, which is itself rewritten (dualVariant) at the end of the same run — `Bootstrap-AprodaProject.ps1` does that settle pull, `Start-Pull.ps1.template` does not. That ordering question is recorded in the E-006 tracker, deliberately **not** changed here. Net-new Aproda file — no `inPlaceEdits` entry needed. | D-4 / D-18 / D-49 | 2026-09-25 |
| `aldc.yaml` | **E-006 T-17**: `external.bcquality.home` value fix `"../../BCQuality-Aproda"` → the constant `".external/bcquality"` (correct by construction for the shipped junction layout once T-29's wrapper mount is the default, D-49); comment rewritten to state it is a **static default / last-resort fallback**, not the authority — the authority is the extension's resolver (`src/bcquality/resolve.ts`), re-resolved and probe-verified per call — and to name how a human checks the real value (`aprodaAldc.bcquality.path` setting, or `Show BCQuality Status`). **Also T-24/B-11(a)**: rewrote the `url` comment, which claimed the install scripts read `url`/`ref`/`pinnedCommit` from here as "the single source of truth" — true only for the upstream `install.{sh,ps1}`, not for the extension's `installOrUpdateBcquality` (hardcoded URL, `pinnedCommit` never checked out); named the deferred fix (T-34). **Also T-24/B-3**: added a note to `evidence.validator`/`evidence.ciWorkflow` stating both files are absent in a consuming project, so the declared config is fork/CI-only. `dualVariant` — no `inPlaceEdits` entry (not an Upstream file). | D-49 | 2026-09-25 |
| `copilot-instructions.md` | **E-006 T-24/B-4**: the "BCQuality evidence: declarative vs falsifiable" section's CI-validation bullet promised *"A hallucinated citation or a drifted pin fails the check"* and a three-way pin cross-check that no longer exists in the validator (B-7). Corrected to state the mechanism is **fork/CI-only** — absent in a consuming project (`tools/bcquality/**` and `.github/workflows/bcquality-evidence.yaml` both missing there, verified against `straub-medical-ag-base`), where the evidence chain stays purely declarative; the surrounding declarative-vs-falsifiable framing is kept, not deleted. Already an `inPlaceEdits` merge-point. | D-2 / D-49 | 2026-09-25 |
| `tools/bcquality/validate_evidence.py` | **E-006 T-24/B-7**: rewrote the module docstring's "Check 1" description — it still claimed a three-way pin cross-check (`aldc.yaml` + both install scripts, *"a drift in any of them fails the build"*) that the code does not perform (`# there is no hardcoded pin to cross-check`, prints a note only). Docstring now matches the code; **no behaviour change**. Upstream-owned file (`fa37cf7`). **Deliberately NOT added to `aproda-sync.json → inPlaceEdits`**: registering it would start shipping the validator to consumer projects, and per **B-6** it currently *passes vacuously* (three independent paths to a false green). T-23 is deferred to Block 5 precisely *because* the validator does not ship — shipping it first would put a green-by-default check into every project, which is the failure class this epic exists to remove. The corrected docstring therefore stays fork-local, which is exactly where the false claim lived. | D-2 / D-49 | 2026-09-25 |
| `docs/bcquality.md` | **E-006 T-24/T-17 (+ S3 review follow-up)**: rewritten to describe the shipped model instead of the superseded one — the `.external/` wrapper + junction layout and why a directly-mounted root can't exclude itself from search (B-9); the resolver's five-rung probe-verified precedence (setting → mounted workspace folder → `$BCQUALITY_HOME` → `<devRoot>/BCQuality-Aproda` → `aldc.yaml → home`); the `#bcquality` tool, status-bar item and `Show BCQuality Status` command; the `aprodaAldc.bcquality.autoReconcile` setting's deliberate asymmetry (gates junction *creation*, never *removal*); and the link-only removal hazard. Also states plainly that CI evidence validation does not run in a consuming project (B-3/B-4), instead of implying it does. Upstream-owned file (`fa37cf7`) — already ships via `aldc.yaml → required.catalog` (registered under T-29), the same mechanism as `agents/index.md`/`docs/copilot-reference.md` — so, per that established precedent, **no separate `aproda-sync.json → inPlaceEdits` entry is needed**. | D-2 / D-49 | 2026-09-25 |
| `tools/aproda-sync/aproda-sync.json` | **E-006 T-25/B-2**: corrected the `neverTouchExceptions` comment's false-ownership claim — it called the BCQuality evidence workflow *"our BCQuality evidence workflow"*; it is Upstream's (`fa37cf7`), not Aproda's. `neverTouchExceptions`'s mechanism itself (B-1, inert) left untouched. **Correction within this same session:** `tools/bcquality/validate_evidence.py` was first added to `inPlaceEdits` and then removed again — registering it would have shipped a vacuously-passing validator (B-6) into every consumer project and contradicted both the `evidence:` comment written one deliverable earlier and T-23's Block-5 deferral rationale. Net-new Aproda file — no `inPlaceEdits` entry needed for this file itself. | D-4 / D-49 | 2026-09-25 |
| `agents/al-conductor.agent.md`, `agents/al-review-subagent.agent.md`, `agents/dredd.agent.md`, `agents/al-triage.agent.md` | **E-006 T-28**: replaced the unexecutable *"fall back to direct root-level access"* clause with the three-rung dual path — `#bcquality` (primary) → `#aldcConfiguration` → `read_file <toolkitRoot>/aldc.yaml` (declared static default, probe before trusting) — now genuinely reachable once T-33 moved `aldc.yaml` into a workspace root. Added `aprodaag.aproda-aldc/bcquality` to the tool allowlist of **all four**. **Review correction:** `al-conductor` carried a bare `aprodaag.aproda-aldc` entry (no tool name, no `/*`), which every other multi-tool grant in this repo writes as `'<id>/*'` — an unverified form with no precedent here, i.e. a candidate for the same silent no-op class as the `al_get_diagnostics` typo (F-15's neighbour). Replaced with the two explicit tool references, so the grant is correct regardless of whether the bare form is supported. All four already `inPlaceEdits` merge points under D-24. **Known drift, deliberately not resolved here:** the agent mirrors under `packages/foundation/agents/**` (out of E-006's scope per the maintainer — it is the *upstream* ALDC extension's packaging source, which Aproda does not ship) and `docs/agents/**` (mkdocs mirror) still carry the pre-T-28 prose. | D-2 / D-24 / D-49 | 2026-09-25 |
| `copilot-instructions.md` | **E-006 T-28**: the "External Knowledge: BCQuality" section rewritten to describe the shipped `.external/` wrapper mount (not a second workspace-root sibling) and the same three-rung dual path. The declarative-vs-falsifiable CI framing (T-24) is unchanged. Already an `inPlaceEdits` merge point. | D-2 / D-49 | 2026-09-25 |
| `tools/aproda-sync/Migrate-AprodaProjectLayout.ps1`, `Initialize-AprodaProject.ps1`, `README.aproda.md`, `templates/gitignore-block.txt` | **E-006 T-35**: new script, chained by `Initialize-AprodaProject.ps1` as **Init 5** (last step, runs on every pull), that migrates a pre-Block-4 project onto the current layout: removes the stale root `aldc.yaml` once `.github/aldc.yaml` exists **and** looks like a real config (non-blank, contains a `toolkitRoot:` key — a byte-length-only check was found to pass on a single-space file and delete the only working config, fixed before release); swaps the legacy sibling BCQuality workspace root (`../BCQuality-Aproda`) for the tracked `.external/` root once it exists; drops `BCQUALITY_HOME` from the workspace file (superseded by the Global VS Code setting, T-17). Idempotent, no-op on an already-migrated project, `-WhatIf`-able. **The `.external/bcquality` junction itself is never touched or recursed into** — only its parent folder's existence is checked. **Honestly:** a `*.code-workspace` carrying `//` comments or custom formatting parses successfully under PowerShell 7 and is rewritten without them — this is a real, irreversible loss, not a hypothetical; the original is saved once, before the first edit, to a sibling `.bak` (added to the `.gitignore` block so the backup of a tracked file is never itself tracked), and the migration report names it. Disclosed plainly in `README.aproda.md`, not hidden behind a flag — no skip option was added, this is disclosure of an intended one-way migration (D-49), not a behaviour change. `Migrate-AprodaProjectLayout.ps1` is a net-new Aproda file, already registered in `aproda-sync.json → includeFiles`. `Initialize-AprodaProject.ps1` documents in its header why it deliberately does not accept `-WhatIf` (its Init 1-4 write unconditionally); the new script can be previewed standalone instead. | D-4 / D-49 | 2026-09-25 |
| `tools/aproda-sync/Sync-AprodaLayer.ps1` | **E-006 T-36 (B-13)**: the framework-file scrape now reads the **source's** `aldc.yaml`, not the destination's. Reading the destination meant a newly registered `required`/`catalog` entry could only resolve on the **next** pull — `aldc.yaml` is itself written (`dualVariant`) at the *end* of the same run — and it also kept **removed** entries alive indefinitely, because the stale destination list outvoted the source. The destination copy is a verbatim copy of the source's anyway (only `toolkitRoot` diverges), so it never carried independent information; reading it merely simulated an autonomy a consuming project does not have. Measured against the reference project: resolved set **132 → 135**, the delta being exactly `agents/index.md`, `docs/copilot-reference.md` and `docs/bcquality.md`, with **nothing dropped**. Side effect: `Bootstrap-AprodaProject.ps1`'s "settle pull" is now redundant (a fresh repo resolves the framework on the first pull); left in place as a harmless idempotent second pass pending a deliberate removal. **Rejected alternative:** adding a settle pull to `Start-Pull.ps1.template` — it doubles every routine pull to paper over an ordering bug, and the generated `Start-Pull.ps1` is git-ignored and machine-local, so it would never reach an existing workstation. Net-new Aproda file — no `inPlaceEdits` entry needed. | D-4 / D-18 / D-49 | 2026-09-25 |
| `tools/aproda-sync/Initialize-AprodaProject.ps1`, `Migrate-AprodaProjectLayout.ps1`, `agents/dredd.agent.md` | **E-006 Block 4 — hotfixes from the end-to-end run 1** (a real pull into a real project, layer ahead of the extension). **(1) Blocker, B-18:** `Start-Pull.ps1` loads the **project's** copy of `Sync-AprodaLayer.ps1`, so the first Block-4 pull runs the *pre-Block-4* engine and then hands over to the *new* `Initialize-AprodaProject.ps1` — which read `templates/external-readme.seed.md` and `Migrate-AprodaProjectLayout.ps1` unguarded and threw, leaving the project half-migrated (empty `.external/`, both BCQuality roots, orphaned `BCQUALITY_HOME`, no `.bak`). Both reads now skip with an explicit "run the pull once more" message. The skew is structural, so the rule is general: **an Init step must never hard-fail because a layer file has not arrived yet.** *Apply Toolkit was never affected* — `Bootstrap-AprodaProject.ps1` runs the **fork's** engine. **(2) Minor:** the migration now also drops the retired sibling root's `search.exclude` / `files.watcherExclude` globs, which it previously left behind permanently. **(3) Major, B-20:** Dredd rendered a degraded audit indistinguishably from a full one — the `not-applicable` outcome lived only in the persisted JSON's `audit.bcquality`/`notes`. Its report contract now requires the BCQuality outcome beside the verdict in **every** reply, with an explicit not-consulted line; Dredd had flagged this against itself, unprompted, during the run. Regression-tested in an isolated fixture reproducing the exact missing-file state: both steps skip, no exception, no empty `.external/` left behind. `dredd.agent.md` is an existing `inPlaceEdits` merge point; the two scripts are net-new Aproda files. | D-2 / D-4 / D-49 | 2026-09-25 |
| `tools/aproda-sync/Initialize-AprodaProject.ps1`, `Migrate-AprodaProjectLayout.ps1`, `templates/Start-Pull.ps1.template` | **E-006 Block 4 — second round of end-to-end findings** (run 1 repeated after the B-18 guards). **(1) B-21, and older than Block 4:** `Initialize-AprodaProject.ps1`'s Init 3 rewrites the tracked `*.code-workspace` through `ConvertTo-Json` — dropping comments and formatting — **with no backup at all**, and always has. The migration's `.bak` only covered the *second* rewrite, so it captured already-modified content and the "your original is saved" promise was never kept. The backup now happens before the **first** rewrite, taken by whichever writer gets there first, still write-once. Verified in a fixture: `.bak` byte-identical to the pristine file, the JSONC comment preserved in it, and the migration's own output unchanged. **(2) B-22:** `Start-Pull.ps1` loaded the **project's** sync engine, which is what produced B-18's version skew; the template now loads engine + manifest from the **fork** (as `Bootstrap-AprodaProject.ps1` does), while `Initialize-AprodaProject.ps1` stays project-side because it anchors its `.git`-walk at its own location. **Honestly: this does not help the projects adopting Block 4 now** — an existing project never receives a refreshed starter (**T-40**), so the first adoption via `Start-Pull.ps1` still needs two pulls. **The maintainer accepted that residual risk on 2026-09-25 with no mitigation**: ~98% of updates (today: all developers but one) go through *Apply Toolkit*, which runs the fork's engine and was never affected. Net-new Aproda files — no `inPlaceEdits` entries needed. | D-4 / D-49 | 2026-09-25 |

| `tools/aproda-vscode-extension/src/**` (`bcquality/install.ts`, `commands/initProject.ts`, `env/gitRoot.ts`, `workspace/bcqualityRoot.ts`, `source/layerSource.ts`, `commands/bcqualityStatus.ts`, `bcquality/aldcConfig.ts`) | **E-006 Block 4 — findings from the end-to-end run 2** (production order, `localFork`, 26/30 clean). **(1) Blocker, B-23:** *Install / Update BCQuality* created a **duplicate clone** and wrote it to the **Global** `aprodaAldc.bcquality.path`, where it outranks every rung in every Aproda project on the machine (confirmed to propagate into `@Dredd`). **The reported diagnosis was wrong** — read and write path share `resolveBcqualityPath()`; the real cause is **timing**: `initializeProject()` resolved BCQuality only *after* `runBootstrap`, and the migration removes the mounted BCQuality root and the workspace `BCQUALITY_HOME` **before** the replacement junction exists, so in that window an existing clone is indistinguishable from a first install. Fixed by capturing the resolution **before** the bootstrap, plus a confirmation naming the path before any clone into a not-yet-existing target. **Deliberately not changed (maintainer, 2026-09-26):** the Global `bcquality.path` write after a successful install — the residual risk is that an accepted wrong path still becomes machine-wide. **A defect the first repair introduced, caught by an independent re-review:** it threaded a single pre-bootstrap repository snapshot into both call sites, but the bootstrap is what *writes* `aldc.yaml`, so a first init would have skipped reconciliation silently; the fallback now re-probes. **(2) B-24(b):** three advisory `showWarningMessage` calls in `ensureLocalFork()` were `await`ed and blocked *Apply Toolkit* until dismissed — now fire-and-forget. **Not swept blanket-wise:** of the 17 remaining awaited message sites, some legitimately gate control flow (`ensureGitRepository`, `confirmGitHubChanges`). **(3) B-25:** `BCQUALITY_HOME` was written for all three platforms regardless of host and read via `get()` (which collapses Workspace over Global) instead of `inspect()?.globalValue`. **(4) B-26:** the repository quick-pick could silently select the BCQuality clone — **reproduced** by a reviewer via a forged `entryPoint` in the clone's own `aldc.yaml`; identity is now taken only from the fixed default marker path, and an unresolvable situation errors instead of guessing. Four regression tests added, **each verified to fail with its fix disabled**. Fork-only tree (`neverTouch`, D-21) — no `inPlaceEdits` entry needed. | D-21 / D-49 | 2026-09-26 |
| `tools/aproda-sync/Sync-AprodaLayer.ps1` | **E-006 B-24(a)**: the noise skip was anchored at the top level, so a nested `.git`/`node_modules` was enumerated in full — **24 018 files instead of 606** on the reference project. Now depth-aware (`(^\|/)(\.git\|node_modules)/`); dry-run 4.9 s. Suspected as the cause of the *Apply Toolkit* hang but **measurement ruled it out** — git time totalled 0.2 s; the hang was B-24(b). Kept as a performance fix on its own merit. Net-new Aproda file — no `inPlaceEdits` entry needed. | D-4 / D-18 | 2026-09-26 |

| `tools/aproda-vscode-extension/src/**` (`bcquality/resolve.ts`, `workspace/bcqualityRoot.ts`, `version/service.ts`, `startup/repositoryInitialization.ts`, `commands/validate.ts`, `commands/initProject.ts`, `commands/bcqualityStatus.ts`) | **E-006 Block 4 — findings from the end-to-end runs 3 and 4** (run 3 = regression on Straub with the wrong `devRoot` deliberately kept; run 4 = the **first-init** path on a repository that had never seen ALDC). B-23…B-26 held in both. **(1) Blocker, B-28:** the resolver discovered **the junction it manages itself** — rung 2 probes `<mountedFolder>/bcquality`, and after migration `.external` *is* a mounted folder — so `.external/bcquality` verified and became the canonical root. Consequences, all observed: a **project-scoped** path written into the **Global** `bcquality.path` / `BCQUALITY_HOME` (poisoning every other project on the machine, B-23's class one step worse), a junction created pointing **at itself** (`ELOOP`), and `verified: true` reported for it. Fixed by canonicalising a verified candidate (`realpath`) before it becomes `root`, plus a self-reference guard in `createBcqualityLink`. **Verified empirically rather than assumed:** `fs.lstat().isSymbolicLink()` returns `true` for a Windows **junction** (real `mklink /J`) — the whole fix rests on that. **Review follow-up:** `reconcileBcquality` wrote the env var from the *caller's* earlier resolution while linking from its own fresh one — two independently timed answers, which is exactly what produced the two mutually inconsistent Global settings observed. **(2) B-29:** `VersionService.readInstalled()` and `repositoryInitialization.ts` still read `<repoRoot>/aldc.yaml`, so **every migrated project reported "not installed" forever** and was offered initialization on every window — T-33 missed two readers; same class as B-10/B-12. Both now use the exported two-rung `resolveConfigurationPath()`. **(3) B-30:** `Validate Installation` died with `spawn npm ENOENT` on Windows — `npm` is a `.cmd` shim. Fixed with `npmExecutable(platform)` at that call site only; **`shell: true` in `run()` was rejected** because it would change argument quoting for every caller, including `git` calls carrying repository URLs. **(4) B-31:** the post-bootstrap BCQuality step logged nothing, making "ran and correctly did nothing" indistinguishable from "never ran" — the shape the broken first repair of B-23 had. The first fix repeated the mistake (the *caller* logged "reconciling" while the function could still return silently); after review the function reports its own outcome. Four regression tests added, **each verified to fail with its fix disabled**. Fork-only tree (`neverTouch`, D-21) — no `inPlaceEdits` entry needed. | D-21 / D-49 | 2026-09-26 |

| `tools/aproda-vscode-extension/src/**` (`process.ts`, `commands/validate.ts`, `bcquality/install.ts`) | **E-006 Block 4 — findings from the end-to-end runs 3.2 and 4.2.** **(1) B-30 was closed on a misdiagnosis and the runs caught it.** `npm.cmd` turned `spawn npm ENOENT` into `spawn EINVAL` — fixed and still broken. Reproduced on this machine (`node -e`, Node 24.21): since Node 18.20.2 (CVE-2024-27980) `spawn` **refuses a `.cmd`/`.bat` without a shell at all**, so the extension name was never the whole problem. Now one **constant** command string under `shell: true` with an **empty** argument array; `run()` gained an **opt-in** `shell` option (default off) so `git` and `node` callers keep their quoting, and the test pins that the validator call does not use it. The earlier review's instinct ("never a global shell") was right; my conclusion from it ("therefore no shell at all") was wrong. **(2) B-32:** `installOrUpdateBcquality` overwrote a user-configured `bcquality.path` with whatever it actually resolved — silently, and into a **Global** setting, so a deliberate override was discarded without a trace and the misconfiguration hid behind a success message. A differing configured path is now left untouched and the mismatch is surfaced. Both covered by regression tests verified to fail with their fix disabled. **(3) Unresolved:** run 4.2 reports the post-bootstrap BCQuality step logging nothing, which the code contradicts — deliberately **not** fixed blind; the next run must read the channel without an intervening reload or extension-host restart. Fork-only tree (`neverTouch`, D-21) — no `inPlaceEdits` entry needed. | D-21 / D-49 | 2026-09-26 |

| `tools/aproda-vscode-extension/src/workspace/bcqualityRoot.ts` | **E-006 B-34 — the B-28 self-reference guard was inverted and shipped in test.5/test.6.** Caught by a clean live repeat: `Refusing to link BCQuality to itself: …\BCQuality-Aproda resolves to …\.external\bcquality.` The guard resolved **both** sides through `realpath`, but a **correct** junction resolves to exactly its target — so the condition was true for every healthy link and the junction stopped being maintained. A link's identity is its **canonical parent plus its own name**, never `realpath(linkPath)`. **Two compounding causes:** the early return also compared paths **case-sensitively**, while `readlink` reports the drive letter as stored (`C:\`) and a resolved candidate can carry it lower-cased (`c:\`) — so the healthy short-circuit never fired and execution reached the broken guard. Both fixed; all path comparisons are now case-insensitive on Windows. **The test passed anyway because it counted `"Linked BCQuality"` messages only** — a refusal was indistinguishable from a no-op. Existence instead of identity, in a test that had been through review; it now asserts that **no error was logged**, that the already-correct branch was taken, and that the link still reaches its target. **Method note:** the same clean repeat also resolved the D18/D35 "silent log" finding — logging was never broken; the channel had been recreated by a reload/extension-host restart before it was read. Fork-only tree (`neverTouch`, D-21) — no `inPlaceEdits` entry needed. | D-21 / D-49 | 2026-09-26 |

| `tools/aldc-validate/index.js`, `tools/aproda-sync/Sync-AprodaLayer.ps1` | **E-006 B-36/B-37 — one glob defect, two places, found by driving the validator's baseline to zero.** **(1) B-36:** a freshly initialized project reported six warnings that can never be resolved there (fork-only artifacts referenced by `skills/index.md` and `readme.aproda.md`'s inventory). Rated *minor* — the index row already says *"fork maintainer only"*, so it is a dangling link, not an unkept promise — but a permanent noise floor makes the validator useless as a gate, which is the same mechanism that kept **B-35** invisible. The validator now derives fork-only status from the sync manifest, **only** in the project layout, so the fork keeps full coverage. **(2) B-37, the real find:** both `Convert-GlobToRegex` (syncer) and the validator's copy rendered `**/*.aproda.*` as `^.*/[^/]*\.aproda\.[^/]*$` — **requiring a slash**, so no root-level path could ever match. In the syncer that means `aproda-sync.json`'s central promise (*"as new .aproda.* artifacts are added — no manifest edit"*) is **false for root-level files**: `CHANGELOG.aproda.md` and `onboarding.aproda.md` silently never shipped, and the manifest's own line-39 comment claiming that omission was fixed on 2026-09-24 (F-14) was wrong. `**/` now spans zero or more directories in both. Dry-run against the reference project: **135 → 137** resolved files, the delta being exactly those two. **Third time in this subsystem that a regex quietly discards files** (B-12, B-36, B-37). **Process note:** the subagent's first B-36 fix shipped that broken glob and thereby classified every root-level `*.aproda.*` as fork-only — trading six false warnings for blindness on `readme.aproda.md`/`decisions.aproda.md`/`site-profile.aproda.md`; its coverage test passed only because the fake path it used contained a slash, and its claimed "0 warnings" came from hand-copying two files into the test project. | D-4 / D-18 / D-49 | 2026-09-26 |
...
### D-49 — The BCQuality clone path is a workstation property: one verifying resolver, a tool channel, and a constant repo-side path

**Context.** The clone location was configured in two **repo-scoped** places — `aldc.yaml → external.bcquality.home` and `*.code-workspace → folders[].path` — which forces every developer onto one relative layout. Measured in the reference project (`straub-medical-ag-base`, 2026-09-24): **three different paths for the same clone, none of them correct**. The documented Aproda install target is `<devRoot>/BCQuality-Aproda`, an absolute path with no fixed relation to any repository, so **no relative value can be correct for everyone**. Three further constraints were measured rather than assumed: agents cannot read outside the workspace roots, so resolving a path is not the same as reaching it; in a consuming project `aldc.yaml` sits outside every workspace root *and* is gitignored, so the agents' documented "fall back to direct root-level access" clause cannot succeed (and neither BCQuality consumer has a terminal in its tool allowlist); and a workspace root **cannot exclude itself** from search, so a mounted clone leaks its 246 example `.al` files into every symbol search. Design trail and the five measured test rounds: `_A-ALDC-Plans/E-006-layer-visibility-and-version-consistency/bcquality.md`.

**Decision.** Four parts, each replacing a guess with a verified value.

1. **Canonical channel = the user setting.** `aprodaAldc.bcquality.path` (VS Code Global/Machine, absolute) is the one value that models "per workstation, across all repos". One resolver in the extension (`src/bcquality/resolve.ts`) answers *where is the clone, and is it really there* with a documented precedence — user setting → a mounted workspace folder containing the entry point → `$BCQUALITY_HOME` → `<devRoot>/BCQuality-Aproda` → `aldc.yaml → home` — and **accepts a candidate only after probing `<root>/skills/entry.md`**. It returns `{ root, resolvedFrom, verified }`; `#aldcConfiguration` exposes `resolvedHome` / `resolvedFrom` / `mounted`. Nothing is cached, so nothing can go silently stale; an unresolvable clone fails visibly (`enabled: true`) or is recorded as an explicit `not-applicable` with the resolver's reason (`auto`).

2. **Reaching the agent: a `#bcquality` tool, plus a dual path.** The extension contributes a language-model tool (`read` + `list`) serving files relative to the resolved clone — the same precedent as `#aldcConfiguration` (D-24), which exists precisely because the repository root need not be a workspace folder. The agent prose keeps a **direct path read as a documented fallback**; tool-only was rejected because it would reproduce the failure class this epic exists to remove — a documented capability that is not in effect.

3. **`aldc.yaml` lives at `toolkitRoot`.** In the fork `toolkitRoot: "."`, so the file already sits there; in a consumer `toolkitRoot: ".github"`, so it belongs at `.github/aldc.yaml` — **today's consumer layout is the rule violation, not the move**. `.github` *is* a workspace root, so the direct read finally works without the extension and without a terminal. Implemented as a **two-rung lookup** (`<toolkitRoot>/aldc.yaml` → `<repoRoot>/aldc.yaml`) so the migration is non-breaking; `home` stays repo-root-relative, like `toolkitRoot`, `plans.root` and `copilotEntrypoint`.

4. **Repo-side layout: a wrapper, never the junction itself.** The seed mounts **`.external/`**, with the clone reached through `.external/bcquality` — an extension-managed directory junction (Windows `mklink /J`, no elevation) or symlink. The wrapper and its tracked `README.md` ship with the toolkit, so the folder exists in every clone and carries its own explanation, including the manual `New-Item -ItemType Junction` / `ln -s` route and the removal rule (**delete the link only** — `Remove-Item -Recurse` can delete through it into the real clone). Because the junction is a *subfolder* of the mounted root, `search.exclude` / `files.watcherExclude` on `bcquality/**` finally resolve — **never `files.exclude`**, which would hide the folder outright and break the read path the agents depend on. With this layout `home` becomes the constant `.external/bcquality`: correct by construction wherever a junction exists, and inert where none does, because the resolver answers first.

**Evidence.** Measured over five rounds in a live consuming project, not reasoned: junction creation needs no elevation and is not blocked by GPO/AppLocker; git treats the junction as one opaque entry and additionally as a nested-repo boundary; AL compilation sees nothing under it (0 diagnostics against 246 `.al` files); `aldc-validate` and `aproda-sync -WhatIf` produce byte-identical output with and without it (control comparison, not a single run); the full consumption chain `entry.md → read.md → do.md → knowledge file` resolves through the mount while search returns zero hits; and the clone survived every cycle byte-for-byte (489 files before and after). Two readings were corrected en route and are kept as corrections rather than overwritten.

**Rejected alternatives.** `$BCQUALITY_HOME` as the *canonical* channel — agents do not read environment variables and neither BCQuality consumer has a terminal; retained only as the script/CI bridge, set as a Global setting. Placeholders in `folders[].path` — verified impossible against the VS Code docs: the schema allows "absolute or relative paths", and substitution exists only in `launch.json`, `tasks.json` and a few named settings. Gitignoring the generated `*.code-workspace`, or shipping a per-user copy beside it — both convert an upstream-shipped, conflict-free file into a permanent D-2 merge point. **The extension writing `home` into `aldc.yaml`** — provably redundant, since the file is gitignored in a consumer and therefore never leaves the machine that already resolves better; and not free, because `Bootstrap-AprodaProject.ps1` rewrites it and the fleet scripts call the same bootstrap, so one `Start-FleetUpdate` would silently reset it. **Syncer scripts reading the VS Code settings file** — no consumer needs it: the bootstrap never touches BCQuality, the fleet scripts skip BCQuality repos by design, and the one script that does need the path already honours `$BCQUALITY_HOME`. A `.gitkeep` to anchor the wrapper — superseded by the README, which anchors it *and* explains it.

**Touches (register rows follow at implementation).** `agents/al-conductor`, `al-review-subagent`, `al-triage`, `dredd` and `copilot-instructions.md` — already in-place edits under D-24, so the tool/dual-path prose extends an existing merge point rather than opening a new one; `aldc.yaml` (dualVariant); `tools/aproda-sync/templates/workspace.seed.jsonc` **and** `Initialize-AprodaProject.ps1`'s fallback workspace writer — two writers, and updating only one would leave projects created through the other on the old layout, the same mechanism as F-14; `tools/aproda-vscode-extension/**`. Migration of existing projects is designed **after** implementation, against the shape that then exists.
