...
**Implementation.** The repository already mixes models per agent (`Claude Sonnet 5 (copilot)`, `GPT-5.6 Terra (copilot)`, `GPT-5.3-Codex (copilot)`), so this follows the established pattern: an `agents/al-translate-subagent.agent.md` with the cheap model in `model:` frontmatter. Display name **`AL Translation Subagent`** — the file uses the short form and the display name the noun form, exactly as `al-implement-subagent` / `AL Implementation Subagent` and `al-review-subagent` / `AL Code Review Subagent` already do. `runSubagent` additionally accepts a per-call `model` override. The subagent **writes `response.json` itself** and returns only a short summary — otherwise the response JSON travels back through the result message and the context-isolation gain is lost.
