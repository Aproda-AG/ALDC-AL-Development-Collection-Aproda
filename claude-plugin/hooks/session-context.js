...
function hasApp(root, depth = 0) {
  ...
  return entries.some(e => e.isFile() && e.name === 'app.json') || entries.some(e =>
...
function context(payload, pluginRoot = path.resolve(__dirname, '..')) {
  ...
  if (!fs.existsSync(path.join(cwd,'aldc.yaml')) && !fs.existsSync(path.join(cwd,'.github/plans')) && !hasApp(cwd)) return null;
