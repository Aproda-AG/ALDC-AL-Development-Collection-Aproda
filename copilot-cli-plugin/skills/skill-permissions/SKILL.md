...
Write the `permissionset` object as AL code with `edit`/`edit` (in VS Code, `AL: Generate permission set` scaffolds one for a human to start from), then refine:
...
Write the XML permission set directly with `edit` (in VS Code, `AL: Generate permission set (as XML)` scaffolds one for a human) when:
...
5. Build: `execute: al compile` — verify no errors in the compiler output
