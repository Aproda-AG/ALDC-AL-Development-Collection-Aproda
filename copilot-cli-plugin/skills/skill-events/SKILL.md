...
Write the subscriber/publisher structures with `edit`/`edit` (in VS Code the `teventsub`/`tevent` snippets scaffold these for a human), then fill the body:
...
1. Build: `execute: al compile` — check for compilation errors (signature mismatches)
