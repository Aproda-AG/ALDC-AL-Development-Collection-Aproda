...
**Migration steps:**
1. Build with `execute: al compile` — signature mismatches produce `AL0482` errors
...
4. Re-verify with `execute: al compile`
...
3. Build: `execute: al compile` — collect all errors
...
2. Full build: `execute: al compile` — zero errors, zero new warnings (produces the `.app`)
