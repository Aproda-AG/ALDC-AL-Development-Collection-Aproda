...
description: |
  Internal AL-aware research and context gathering subagent for Business Central development. Only invoked by al-conductor via agent tool. Returns structured findings to Conductor for plan creation.
tools:
  - read
  - search
  - edit
...
**Use These Tools (Copilot CLI harness):**
...
- `execute: git diff` / `git log` - Review recent modifications to AL code
- `execute: git log` (and `web` for public repos) - Understand development history and team patterns
