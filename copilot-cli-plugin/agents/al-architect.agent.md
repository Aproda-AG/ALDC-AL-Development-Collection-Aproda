...
- **MEDIUM/HIGH complexity** → Use the agent tool to delegate to agent `al-conductor` with context: "Implement the approved architecture using TDD orchestration. Architecture contract: .github/plans/{req_name}/{req_name}.architecture.md"
- **LOW complexity** → Use the agent tool to delegate to agent `al-developer` with context: "Implement simple feature directly. Spec: .github/plans/{req_name}/{req_name}.spec.md"
