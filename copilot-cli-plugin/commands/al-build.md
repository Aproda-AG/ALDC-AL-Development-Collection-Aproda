---
description: |
  Build, package, and deploy AL extensions to Business Central environments. Use when you need to build, compile, package, publish, or deploy an AL extension.
---
...
1. **Build**: `execute: al compile` (single project) or `al workspace compile` (multi-project) to produce the `.app`
...
### Testing Environment
1. **Build**: `execute: al compile` with full validation (read all warnings/errors)
...
1. **Build**: `execute: al compile` with strict validation
