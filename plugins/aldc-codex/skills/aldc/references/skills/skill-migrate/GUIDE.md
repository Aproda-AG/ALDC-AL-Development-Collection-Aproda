...
**Migration steps:**
1. Build with `shell: al compile` — signature mismatches produce `AL0482` errors
...
4. Re-verify with `shell: al compile`
...
3. Build: `shell: al compile` — collect all errors
...
2. Full build: `shell: al compile` — zero errors, zero new warnings (produces the `.app`)
