...
function bodyFor(text) {
  return text.replace(/Claude Code/g,'Codex')
    .replace(/`?\bTodoWrite\b`?/g,'the available planning tool (or the plan document)')
    .replace(/\bTask tool\b/g,'subagent delegation tool').replace(/`Task`/g,'the available subagent delegation tool')
    .replace(/\bBash\b/g,'shell').replace(/`(?:Read|Glob|Grep)`/g,'the available file reading/search tool')
    .replace(/`(?:Write|Edit)`/g,'the available file editing tool')
    .replace(/`(?:WebSearch|WebFetch)`/g,'the available web research tool')
